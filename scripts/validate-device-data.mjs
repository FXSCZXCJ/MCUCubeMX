// 器件数据校验：64 引脚完整性、AF 表与引脚定义一致性、EXTI 分组正确性。
import { readFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import path from 'node:path'

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const dev = 'gd32l233rct6'
const read = (name) => JSON.parse(readFileSync(path.join(root, 'data', 'devices', dev, name), 'utf8'))
...
// ---- package.json ----
const pins = pkg.pins
if (!Array.isArray(pins) || pins.length !== 64) fail(`pins 数量应为 64，实际 ${pins?.length}`)
...
for (const pin of pins) {
  ...
  if (!['IO', 'POWER'].includes(pin.type)) fail(`${pin.name}: 非法 type ${pin.type}`)
}
for (const side of sides) {
  if (sideCounts[side] !== 16) fail(`side ${side} 应有 16 个引脚，实际 ${sideCounts[side]}`)
}
...
for (const entry of exti.entries) {
  ...
  const expect =
    entry.line <= 4 ? `EXTI${entry.line}` : entry.line <= 9 ? 'EXTI5_9' : 'EXTI10_15'
...
console.log(
  `OK: ${pkg.device} ${pkg.package} | ${pins.length} pins | ${af.entries.length} AF entries | ${exti.entries.length} EXTI entries`,
)
