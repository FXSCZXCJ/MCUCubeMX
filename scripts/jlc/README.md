...
8. 反向同步：点击 **同步到 EDA**，按引脚现有连接方式处理（已连端口→更新/更换端口；线段→按所选模式处理；未连→新增端口），同样先弹**变更对比**对话框确认。
9. 面板顶部可选择**同步方式**（记住选择）：
   - **线段模式**：线段→改线段网络名（不删线段不放端口）；未连线跳过；
   - **端口模式**：未连线→新增端口，端口→更新/更换；已有线段不删除（跳过）；
   - **转化为网络端口**：已有线段→删除并放置 IN/OUT 端口；未连线→新增端口。
   - **转化为线段**：端口→删除并放置线段；未连线→放置线段；已有线段→改网络名。
